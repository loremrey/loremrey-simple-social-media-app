...
export default function CreatePostSubusRoute({ params }: { params: { id: string } }) {
	const [imageUrl, setImageUrl] = useState<null | string>(null);
	const [json, setJson] = useState<null | JSONContent>(null);
	const [title, setTitle] = useState<null | string>(null);
	const createPostSubus = async (formData: FormData) => {
			...
			<div className="w-[65%] flex flex-col gap-y-5">
				...
				<Tabs defaultValue="post" className="w-full">
					...
					<TabsContent value="post">
						<Card>
							<form action={createPostSubus}>
								<input type="hidden" name="imageUrl" value={imageUrl ?? undefined} />
								<input type="hidden" name="subusName" value={params.id} />
								<CardHeader>
									<Label>Title</Label>
			...
			<div className="w-[35%]">
				<Card className="flex flex-col p-4">
					<div className="flex items-center gap-x-2">
						...
						<h1 className="font-medium">Posting to Subus</h1>
					</div>
